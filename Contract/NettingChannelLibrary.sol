...
library NettingChannelLibrary {
    ...
    struct Participant
    {
		...
		// The unlocked amount by withdraw and withdrawDelegate  
		// Fix or add by SmartRaiden
        ...
        public
    {
		...
		// Fix or add by SmartRaiden
		if(transferred_amount>self.participants[closer_index].transferred_amount)		
        ...
        public
    {
        ...
        // Update the structure of the closer with its data provided by the
        // counterparty
		closer_index = index_or_throw(self, self.closing_address);
		if(self.participants[closer_index].transferred_amount<transferred_amount){
			self.participants[closer_index].nonce = nonce;
			self.participants[closer_index].locksroot = locksroot;
			self.participants[closer_index].transferred_amount = transferred_amount;
			self.Agent.length+=1;
			self.Agent[length-1]=msg.sender;
		}
    }
        ...
        public
    {
        ...
        // accounted for.
        counterparty.unlock_amount += amount;
    }
        ...
        public
    {
        ...
        index =index_or_throw(self, self.closing_address);
        Participant storage counterparty = self.participants[index];
        ...
        public
    {
		...
		//Add unlock amount to transfered_amout
		closing_party.transferred_amount+=closing_party.unlock_amount;
		counter_party.transferred_amount+=counter_party.unlock_amount;
        ...
        //pay the commission
		counter_amount=counter_amount-Agents.length*Price;
		
		for(i=0;i<Agents.length;i++){
			require(self.token.transfer(Agents[i], Price));
		}
